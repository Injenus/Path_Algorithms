"""
Эксперименты с пошаговой отрисовкой
sampled_map превращается в 4д массив - отдельный 3д массив
...
r_bot = 0.25  # диаметр робота в метрах
...
max_d_rrt = real_dl * 5.5
max_samples_rrt = 2000

animat_map = np.ndarray(shape=(
    1, sampled_map.shape[0], sampled_map.shape[1], sampled_map.shape[2]),
    dtype=float)
animat_map[0] = sampled_map
...
def draw_node(node, value,ind):
    animat_map=np.append(animat_map, [animat_map[-1]], axis=0)
    if value == 0.25:
        if not animat_map[-1][int(node) // animat_map[-1].shape[1]][
            int(node) % animat_map[-1].shape[1]][2]:
            animat_map[-1][int(node) // animat_map[-1].shape[1]][
                int(node) % animat_map[-1].shape[1]][2] = value
            return True
        else:
            return False
    elif value == 0.5:
        if not animat_map[-1][int(node) // animat_map[-1].shape[1]][
            int(node) % animat_map[-1].shape[1]][2] or \
                animat_map[-1][int(node) // animat_map[-1].shape[1]][
                    int(node) % animat_map[-1].shape[1]][2] == 0.25:
            animat_map[-1][int(node) // animat_map[-1].shape[1]][
                int(node) % animat_map[-1].shape[1]][2] = 0.5
            return True
        else:
            return False
        ...
        if curr_node[1] == float('inf'):
            ...
            for i, node in enumerate(to_vist):
                ...
                if curr_label < unvisited_nodes[cur_ind][1]:
                    ...
                    if draw_node(node, 0.25,k):
                        node_counter += 1
            unvisited_nodes = np.delete(unvisited_nodes, 0, axis=0)
            if draw_node(curr_node[0], 0.5,k):
                node_counter += 1
        k += 1
...
def rrt(s, f, dist_f, max_d=max_d_rrt):
    ...
    def search_path(max_d=max_d):
        nonlocal pathes, node_counter, rand_node_coun, isNotInToler
    ...
    print('< В дереве вершин: {} >'.format(node_counter))
    path = pathes[f]
    len_path = 0
...
def rrt_star(s, f, dist_f, max_d=max_d_rrt):
    ...
    def search_path(max_d=max_d):
        nonlocal pathes, node_counter, rand_node_coun, isNotInToler, diffr
        while isNotInToler or rand_node_coun < max_samples:
            ...
            while isGoing:  # пока недалеко или не дойдём до выборки или препятствия
                ...
                if ((xy_child[0] - xy_parent[0]) ** 2 + (
                    ...
                    if child_node not in T.nodes():
                        ...
                        child_neighbs = get_neighbs(child_node, 9 * real_dl)
                        # cost_min = cost_to_node(parent_node) + \
                        #            G.edges[[parent_node, child_node]]['weight']
                        ...
                        for neighb in child_neighbs:
                            if nx.is_path(G, [child_node, neighb]):
...
def check_ru(name, i, j):
    if not sampled_map[i - 1][(j + 1) % len(sampled_map[i])][2]:  # пв
        if j == len(sampled_map[i]) - 1:
...
while True:
        ...
        if event.type == pygame.QUIT:
            ...
            if isSetObj[0] * isSetObj[1]:
                if keys[pygame.K_d]:
                    ...
                    isNewEnvir = True
                    isInProgress = True
                    if animat_map.shape[0]>1:
                        animat_map=np.delete(animat_map, animat_map[1:])
                    path = dijkstra(
    ...
    screen.fill((0, 0, 0))
    for i in range(animat_map.shape[1]):
        for j in range(animat_map.shape[2]):
            # x = sampled_map[i][j][0] - 0.5 ** 0.5 * dl + (
            #         dl + margin) * j + offset[0]
            # y = -sampled_map[i][j][1] - 0.5 ** 0.5 * dl + (
            #         dl + margin) * i + offset[1]
            x = int(animat_map[s][i][j][0] - animat_map[s][0][0][0] + (
                    dl + margin) * j + offset[0])
            y = int(-animat_map[s][i][j][1] + animat_map[s][0][0][1] + (
                    dl + margin) * i + offset[1])
            if animat_map[s][i][j][2] == 0:
                color = blue
            elif animat_map[s][i][j][2] == 1:
                color = red
            elif animat_map[s][i][j][2] == 0.5:
                # color = (0, 168, 192)
                color = green_dark
            elif animat_map[s][i][j][2] == 0.25:
                color = green_light
            else:
                color = blue
                # color = gray
